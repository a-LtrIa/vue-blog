...
router.get('/slug/:slug', (req, res) => {
  ...
  res.json({ ...post, tags, view_count: post.view_count + 1 })
})

router.post('/', authMiddleware, (req, res) => {
  const { title, slug, content, excerpt, cover_image, category_id, status = 'draft', tags = [] } = req.body
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(title, slug, content, excerpt, cover_image, category_id, status)
...
router.put('/:id', authMiddleware, (req, res) => {
  const { title, slug, content, excerpt, cover_image, category_id, status, tags } = req.body
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(title, slug, content, excerpt, cover_image, category_id, status, req.params.id)
